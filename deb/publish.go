...
func (p *PublishedRepo) String() string {
	...
	if p.Description != "" {
		extras = append(extras, fmt.Sprintf("description: %s", p.Description))
	}
...
// GetDescription returns default or manual Description:
func (p *PublishedRepo) GetDescription() string {
	if p.Description == "" {
		return "Generated by aptly"
	}
